...
local function drawMapNames()
	if exports.hud_maximap:isPlayerMapVisible() then
		...
		for k,v in ipairs(getElementsByType("blip")) do
			...
			if isElement(attached) and getElementType(attached)=="player" then
				...
				if ((showType == "near") and (getDistanceBetweenPoints2D(lX, lY, px,py) < 500)) or (showType == true) then
					local x,y        = exports.hud_maximap:getMapFromWorldPosition(px,py)	-- Get the blip position on the map
					x                = x+xOffset										-- X for the nametag
					y                = y-yOffset										-- Y for the nametag
					local pname      = getPlayerName(attached)							-- Player name
					local nameLength = dxGetTextWidth(pname,textScale,textFont, true)	-- Width of the playername
