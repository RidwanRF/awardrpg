...
exports.hud_maximap:setPlayerMapImage("images/sattelite.jpg",-4606,3000,3994,-3000)
	-- Create a small GUI, for managing the maximap
	...
	-- local r,g,b,a = exports.hud_maximap:getPlayerMapColor() -- Get the current player map color so we can update our scrollbar with this
	...
	-- guiRadioButtonSetSelected(radioRadar,exports.hud_maximap:getPlayerMapImage()==defaultImage)  -- Make it clear that "Radar" is the default option by automatically highlighting it, if default image is showing
	
	-- local mapVisible=exports.hud_maximap:isPlayerMapVisible()
...
function continueToggleCPanel()
	-- local toggle=exports.hud_maximap:isPlayerMapVisible() -- See if the maximap is visible
...
function toggleMap()
		 -- Change the image to our glorious, pretty high definition (1 pixel/2 meter) sattelite image!
end
...
function changeColor()
	...
	if srP~=scrollRedPos or sgP~=scrollGreenPos or sbP~=scrollBluePos or saP~=scrollAlphaPos then -- If something's changed in comparison with last frame then
		exports.hud_maximap:setPlayerMapColor(srP*2.55,sgP*2.55,sbP*2.55,saP*2.55) -- Set the map color accordingly
...
function changeParameters()  -- This function should be veeeeery obvious so I'll just shut my trap
	...
	if newMovSpeed then
		exports.hud_maximap:setPlayerMapMovementSpeed(newMovSpeed)
	end
	
	if newZoom then
		exports.hud_maximap:setPlayerMapZoomFactor(newZoom)
	end
	
	if newZoomRate then
		exports.hud_maximap:setPlayerMapZoomRate(newZoomRate)
	end
	
	if newMinZoom then
		exports.hud_maximap:setPlayerMapMinZoomLimit(newMinZoom)
	end
	
	if newMaxZoom then
		exports.hud_maximap:setPlayerMapMaxZoomLimit(newMaxZoom)
	end
end