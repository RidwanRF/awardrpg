...
function printLocation(button,state,aX,aY)
	if (button=="right") and (state=="down") and exports.hud_maximap:isPlayerMapVisible() then	-- Only trigger this if it's due to a right-click and the state is "down", we don't want too many triggers now do we? :P
		...
		local x,y=exports.hud_maximap:getWorldFromMapPosition(aX,aY)	-- We want to get the world position in comparison with the map, so we get fetch that and give the cursor position for it to calculate
