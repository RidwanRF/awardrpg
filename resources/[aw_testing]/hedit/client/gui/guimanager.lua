...
function toggleEditor ( )
    ...
    if pVehicle then
		...
		-- ЧЕНДЖ
		if getElementData(pVehicle, "JobVeh") then -- Чтобы нельзя было открыть на рабочей машине
			return false
		end
		
		 -- Чтобы нельзя было открыть вне СТО
		if not isVehicleInsideRepairStation(pVehicle) then
			if not pData.isadmin then
				guiCreateWarningMessage ( getText ( "notInRepair" ), 1)
				return false
			end
		end
		
		if (getVehicleType(pVehicle) == "Boat") then	-- Чтобы не открывалось на лодках
			return false
		end
		-- /ЧЕНДЖ
