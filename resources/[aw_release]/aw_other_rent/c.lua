...
function PrivateVehicles:constructor()
    ...
    self.time = {}
    self.vehicle = nil
    self.font = dxCreateFont("assets/bold.ttf", 11)
...
function PrivateVehicles:onClientVehicleExit(vehicle, player, seat)
    if player == localPlayer and seat == 0 and self.vehicle == vehicle then
        self.update = 0
		exports.aw_interface_notify:showInfobox("info", "Аренда транспорта", "Вы покинули транспорт, у вас есть 60 секунд или аренда завершится.", getTickCount(), 8000)
        -- Уведомление "Вы покинули Т/С, у вас есть 60 секунд как Т/С пропадет."
...
function PrivateVehicles:onClientRender()
    ...
    for vehicle, data in pairs(self.elements) do
        ...
        if vx and vy and clear and dist < 40 then
             ...
             dist = dist/30
              dxDrawText(data.owner and string.format("#908ef4Транспорт для новичков\n#ffffffВ аренде у : #ffffff(%s)", data.owner) or "#908ef4Транспорт для новичков\n#ffffff(Со первого уровня)", vx, vy, vx, vy, tocolor(255, 255, 255, 255 * alpha), 1/zoom - dist, self.font, "center", "center", false, false, false, true)
        end
    end
    if self.vehicle and isElement(self.vehicle) then
        if self.time and self.time.start and self.time.finish then
            ...
            if progress <= 1.0 and self.update ~= math.floor(time) then
                ...
                if self.notification then
                  exports.aw_interface_notify:showInfobox("info", "Аренда транспорта", "Вы вышли из транспорта, он исчезнет через: %ds.", time, getTickCount(), 8000)
                end
