...
local moonPhase = 0
local weather = 1
local weatherNext = math.random(1, 10)
local weatherTime = 0
setWeather(weather)
setTimer(function ()
if not isElement(shaderTable.skyboxTropos) then return end
local hour, minutes = getTime()
minutes = (1/ 59) * minutes
dxSetShaderValue(shaderTable.skyboxTropos, "GameTime", hour+minutes)
--dxSetShaderValue(shaderTable.skyboxTropos, "WeatherAndTime", Vector3(weather, weatherNext, weatherTime / 30))
dxSetShaderValue(shaderTable.skyboxTropos, "currentWeather", getWeather())
-- weatherTime = weatherTime + 1
end, 1000, 0)
-- setTimer(function ()
-- 	weather = weatherNext
-- 	weatherNext = math.random(1, 10)
-- 	weatherTime = 0
-- 	setWeather(weather)
-- end, 30000, 0)
shaderHideText = [[
    float4 JustColor() : COLOR0
    {
...
shaderTable.hide = dxCreateShader(shaderHideText, 0, 0, false, "world")
engineApplyShaderToWorldTexture(shaderTable.hide, "coronastar")
engineApplyShaderToWorldTexture(shaderTable.hide, "coronamoon")
...
textureTable.skybox = dxCreateTexture ( "tex/skybox.png", "dxt5" )	
textureTable.noise1 = dxCreateTexture ( "tex/Noise.png", "dxt5" )	
textureTable.noise2 = dxCreateTexture ( "tex/Noise2.png", "dxt5" )	
textureTable.noise4 = dxCreateTexture ( "tex/Noise4.png", "dxt5" )	
textureTable.noise5 = dxCreateTexture ( "tex/Noise5.png", "dxt5" )		
textureTable.stars = dxCreateTexture ( "tex/Stars.png", "dxt5" )
textureTable.moon1 = dxCreateTexture ( "tex/Moon.png", "dxt5" )	
...
dxSetShaderValue ( shaderTable.skyboxTropos, "sNs", textureTable.noise1 )
dxSetShaderValue ( shaderTable.skyboxTropos, "sNs2", textureTable.noise2 )
dxSetShaderValue ( shaderTable.skyboxTropos, "sNs4", textureTable.noise4 )	
dxSetShaderValue ( shaderTable.skyboxTropos, "sNs5", textureTable.noise5 )		
dxSetShaderValue ( shaderTable.skyboxTropos, "sStars", textureTable.stars )	
dxSetShaderValue ( shaderTable.skyboxTropos, "sMoon", textureTable.moon )
dxSetShaderValue ( shaderTable.skyboxTropos, "sMoon1", textureTable.moon1 )
...
if isElement(shaderTable.hide) then
	engineRemoveShaderFromWorldTexture(shaderTable.hide, "coronastar")
	engineRemoveShaderFromWorldTexture(shaderTable.hide, "coronamoon")
end
for _,part in ipairs(effectParts) do
	destroyElement(part)
end
