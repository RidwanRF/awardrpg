...
function aMessageBox ( type, message, action, vOne, vTwo, vThree )
	...
	if ( aMessageForm == nil ) then
		...
		aMessageYes		= guiCreateButton ( 120, 70, 100, 20, "Да", false, aMessageForm )
		aMessageNo		= guiCreateButton ( 240, 70, 100, 20, "Нет", false, aMessageForm )
		aMessageOk		= guiCreateButton ( 160, 70, 100, 20, "Ок", false, aMessageForm )
	...
	if ( type == "warning" ) then guiSetVisible ( aMessageWarning, true ) guiSetText ( aMessageForm, "Внимание" )
	elseif ( type == "question" ) then guiSetVisible ( aMessageQuestion, true ) guiSetText ( aMessageForm, "Подтверждение" )
	elseif ( type == "error" ) then guiSetVisible ( aMessageError, true ) guiSetText ( aMessageForm, "Ошибка" )
...
function runMessageAction(action)
	...
	elseif (action == "setdonateminus") then
		triggerServerEvent("aPlayer", localPlayer, varOne, "setdonateminus")
	elseif (action == "setbankminus") then
		triggerServerEvent("aPlayer", localPlayer, varOne, "setbankminus")
