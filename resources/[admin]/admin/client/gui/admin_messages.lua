...
function aViewMessages ( player )
	if ( aMessagesForm == nil ) then
		...
		aMessagesForm	= guiCreateWindow ( x / 2 - 250, y / 2 - 125, 620, 250, "Репорты", false )
		...
		aMessagesRead	= guiCreateButton ( 0.86, 0.20, 0.12, 0.09, "Посмотреть", true, aMessagesForm )
...
function aClientMessagesClick ( button )
	if ( button == "left" ) then
		...
		elseif ( source == aMessagesRead ) then
			local row = guiGridListGetSelectedItem ( aMessagesList )
			if ( row == -1 ) then aMessageBox ( "Warning", "Сообщение не выбрано!", nil )
			else
				local id = guiGridListGetItemText ( aMessagesList, row, 1 )
				aViewMessage ( tonumber ( id ) )
			end
		elseif ( source == aMessagesDelete ) then
			local row = guiGridListGetSelectedItem ( aMessagesList )
			if ( row == -1 ) then aMessageBox ( "Warning", "Сообщение не выбрано!" )
			else
				local id = guiGridListGetItemText ( aMessagesList, row, 1 )
