...
function aInputBox ( title, message, default, action, vOne, vTwo )
	if ( aInputForm == nil ) then
		...
		aInputOk		= guiCreateButton ( 45, 80, 100, 20, "Применить", false, aInputForm )
		aInputCancel		= guiCreateButton ( 155, 80, 100, 20, "Отмена", false, aInputForm )
...
function aInputBoxClick ( button )
	if ( button == "left" ) then
		if ( source == aInputOk ) then
            ...
            --
            elseif (aInputAction == "setDonateMoney") then
				triggerServerEvent("aPlayer", localPlayer, varOne, "setDonateMoney", guiGetText(aInputValue))
			elseif (aInputAction == "setBankMoney") then
				triggerServerEvent("aPlayer", localPlayer, varOne, "setBankMoney", guiGetText(aInputValue))
			elseif (aInputAction == "uplevel") then
				triggerServerEvent("aPlayer", localPlayer, varOne, "uplevel", guiGetText(aInputValue))
            elseif (aInputAction == "setsex") then
				triggerServerEvent("aPlayer", localPlayer, varOne, "setsex", guiGetText(aInputValue))
			elseif (aInputAction == "setparkmesta") then
				triggerServerEvent("aPlayer", localPlayer, varOne, "setparkmesta", guiGetText(aInputValue))
			elseif (aInputAction == "setparkmesta") then
				triggerServerEvent("aPlayer", localPlayer, varOne, "setparkmesta", guiGetText(aInputValue))
			elseif (aInputAction == "minuspark") then
				triggerServerEvent("aPlayer", localPlayer, varOne, "minuspark", guiGetText(aInputValue))
            --
...
function aBanInputBoxFinish ()
	...
	if not bUseIP and not bUseSerial then
		aMessageBox ( "error", "Не выбран тип" )
		return
	end

	if seconds == false then
		aMessageBox ( "error", "Не выбрана длительность!" )
		return
	end
