...
function refreshBalance(plr)
	if (not isElement(plr)) then return end
	triggerClientEvent(source, "setPlayerDetails", source, plr, 
		{
			["RUB"] = exports.bank:getPlayerBankMoney(plr, "RUB"),
			["EUR"] = exports.bank:getPlayerBankMoney(plr, "EUR"),
			["USD"] = exports.bank:getPlayerBankMoney(plr, "USD"),
			["DONATE"] = exports.bank:getPlayerBankMoney(plr, "DONATE")
	    }
	)
end
addEvent("refreshBalance", true)
addEventHandler("refreshBalance", root, refreshBalance)
...
function notifyPlayerLoggedIn(player)
	outputChatBox ( "* Нажмите P чтобы открыть админ-панель", player, 153, 132, 243, true )
	local unread = 0
	for _, msg in ipairs ( aReports ) do
		unread = unread + ( msg.read and 0 or 1 )
	end
	if unread > 0 then
		outputChatBox( unread .. " непрочитанное сообщение" .. ( unread==1 and "" or "s" ), player, 255, 0, 0 )
	end
end
...
function aPlayerInitialize ( player )
	...
	if ( not isValidSerial ( serial ) ) then
		outputChatBox ( "ОШИБКА: "..getPlayerName ( player ).." - Неверный серийный номер." )
...
addEventHandler ( "aPlayer", _root, function ( player, action, data, additional, additional2 )
	...
	if ( hasObjectPermissionTo ( source, "command."..action ) ) then
		...
		elseif ( action == "ban" ) then
			...
			if bUseSerial then
				outputChatBox ( "Вы заблокировали игрока по серийному номеру " .. getPlayerSerial( player ), source, 255, 100, 70 )
				setTimer ( addBan, 100, 1, nil, nil, getPlayerSerial(player), source, reason, seconds or 0 )
			else
				outputChatBox ( "Вы заблокировали игрока по IP " .. getPlayerIP( player ), source, 255, 100, 70 )
		...
		elseif ( action == "setmoney" ) then
			mdata = data
			exports.aw_server_logs:log("setMoneyAdminPanel", "Игрок: "..getPlayerName (source)..": Выдал "..getPlayerName (player).." Наруки: "..data.." руб.")
		...
		-------------------------------------
		--. Выдача и обнуление валюты в банке
		elseif ( action == "setDonateMoney" )  then 
			mdata = data
			if ( not data ) then
				outputChatBox ( "Недействительные данные", source, 255, 0, 0 )
				action = nil
			else
				exports.aw_server_logs:log("DonateAdminPanel", "Игрок: "..getPlayerName (source)..": Выдал "..getPlayerName (player).." DC-Coins: "..data.." DC.")
				exports.bank:givePlayerBankMoney(player, data, "DONATE")
				--exports.logs:logMessage("Администратор "..getPlayerName(source).." выдал "..convertNumber(data).." донат ед. игроку"..getPlayerName(player), 3)
				outputChatBox ("#40bf80Вы успешно выдали #d9d9d9"..convertNumber(data).." донат ед. #40bf80игроку #ffd11a"..getPlayerName(player), source, 255, 0, 0, true )
				outputChatBox ("#40bf80Ваш донат счет был пополнен на #d9d9d9"..convertNumber(data).." донат ед.", player, 255, 0, 0, true )
			end 
        elseif ( action == "setBankMoney" ) then
			mdata = data
			if ( not data ) then
				outputChatBox ( "Недействительные данные", source, 255, 0, 0 )
				action = nil
			else
				exports.aw_server_logs:log("BankMoneyAdminPanel", "Игрок: "..getPlayerName (source)..": Выдал "..getPlayerName (player).." BankMoney: "..data.." руб.")
				exports.bank:givePlayerBankMoney(player, data, "RUB")
				--exports.logs:logMessage("Администратор "..getPlayerName(source).." выдал "..convertNumber(data).."$ игроку "..getPlayerName(player), 3)
				outputChatBox ("#40bf80Вы успешно выдали #d9d9d9"..convertNumber(data).."₽ #40bf80игроку #ffd11a"..getPlayerName(player), source, 255, 0, 0, true )
				outputChatBox ("#40bf80Ваш банк. счет был пополнен на #d9d9d9"..convertNumber(data).."₽", player, 255, 0, 0, true )
			end
		--
		elseif ( action == "setdonateminus" )  then
			mdata = data
			if ( data ) then
				outputChatBox ( "Недействительные данные", source, 255, 0, 0 )
				action = nil
			else
				exports.aw_server_logs:log("DonateAdminPanel", "Игрок: "..getPlayerName (source)..": Забрал "..getPlayerName (player).." DC-Coins: "..exports.bank:getPlayerBankMoney(player, "DONATE").." DC.")
				exports.bank:takePlayerBankMoney(player, exports.bank:getPlayerBankMoney(player, "DONATE"), "DONATE")
				--exports.logs:logMessage("Администратор "..getPlayerName(source).." аннулировал донат ед. у игрока "..getPlayerName(player), 3)
				outputChatBox ("#40bf80Вы успешно аннулировали донат ед. #40bf80игрока #ffd11a"..getPlayerName(player), source, 255, 0, 0, true )
				outputChatBox ("#40bf80У Вас аннулировали донат ед.", player, 255, 0, 0, true )
			end
		elseif ( action == "setbankminus" )  then
			mdata = data
			if ( data ) then
				outputChatBox ( "Недействительные данные", source, 255, 0, 0 )
				action = nil
			else
				exports.aw_server_logs:log("BankMoneyAdminPanel", "Игрок: "..getPlayerName (source)..": Забрал "..getPlayerName (player).." BankMoney: "..exports.bank:getPlayerBankMoney(player, "RUB").." руб.")
				exports.bank:takePlayerBankMoney(player, exports.bank:getPlayerBankMoney(player, "RUB"), "RUB")
			--	exports.logs:logMessage("Администратор "..getPlayerName(source).." аннулировал весь банк у игрока "..getPlayerName(player), 3)
				outputChatBox ("#40bf80Вы успешно аннулировали весь банк у игрока #ffd11a"..getPlayerName(player), source, 255, 0, 0, true )
				outputChatBox ("#40bf80У Вас аннулировали банк. счет", player, 255, 0, 0, true )
			end
        --. Парк. места 
        elseif ( action == "setparkmesta" )  then
			mdata = data
			if ( not data ) then
				outputChatBox ( "Недействительные данные", source, 255, 0, 0 )
				action = nil
				...
				local accData = getAccountData (acc, "donat:parking") or 0
				setAccountData (acc, "donat:parking", accData + data)
				exports.car_system:updateVehicleInfo(player)
				--
				outputChatBox ("#e6b800Вы успешно изменили парк. места у игрока #e6e6e6"..getPlayerName(player), source, 255, 0, 0, true )
				outputChatBox ("#ff5c33Администратор добавил Вам число парк. мест #e6e6e6[+"..data.."]", player, 255, 0, 0, true )
	        end
		--. Парк. места 
        elseif ( action == "minuspark" )  then
			mdata = data
			if ( not data ) then
				outputChatBox ( "Недействительные данные", source, 255, 0, 0 )
				action = nil
				...
				--
				outputChatBox ("#e6b800Вы успешно изменили парк. места у игрока #e6e6e6"..getPlayerName(player), source, 255, 0, 0, true )
				outputChatBox ("#ff5c33Администратор изменил число парк. мест в гараже #e6e6e6[-"..data.."]", player, 255, 0, 0, true )
	        end
		...
		elseif ( action == "warp" ) or ( action == "warpto" ) then
    		function warpPlayer ( p, to )
				if PlayerAccessWarp[to] then outputChatBox ( "* Админ отключил телепорт к себе!", p, 255, 168, 0 ) return end
				function warp ( p, to )
...
function warpaccess (pl)
if not PlayerAccessWarp[pl] then PlayerAccessWarp[pl] = false end
PlayerAccessWarp[pl] = not PlayerAccessWarp[pl]
if PlayerAccessWarp[pl] then
outputChatBox ("* Вы отключили телепорт к себе!", pl, 255, 255, 255, true)
else
outputChatBox ("* Вы включили телепорт к себе!", pl, 255, 255, 255, true)
end
end
addCommandHandler ("stopwarp", warpaccess, true)
	...
	if ( action == "new" ) then
		...
		for _, p in ipairs ( getElementsByType ( "player" ) ) do
			if ( hasObjectPermissionTo ( p, "general.adminpanel" ) ) then
				outputChatBox( "Новое сообщение администратора от " .. aReports[id].author .. " (" .. aReports[id].subject .. ")", p, 255, 0, 0 )
			end
		end
		outputDebugString(string.format("[ADMIN][REPORT] %s (acc %s money %i) wrote report. Subj: \"%s\", text: \"%s\"",
			aReports[id].author, getPlayerAccountName(source), getPlayerMoney(source), aReports[id].subject, aReports[id].text
		))
